...
export const Navbar=({isProductList=false})=> {
  ...
  return (
    <AppBar position="sticky" sx={{backgroundColor:"black",boxShadow:"none",color:"white"}}>
        <Toolbar sx={{p:1,height:"6rem",display:"flex",justifyContent:"space-between"}}>
          ...
          <Typography variant="h6" noWrap component="a" href="/" sx={{ mr: 2, display: { xs: 'none', md: 'flex' },fontWeight: 700, letterSpacing: '.3rem', color: 'inherit', textDecoration: 'none', }}>
            ...
            {loggedInUser.isAdmin && <Typography variant='contained'>ADMIN CONSOLE</Typography>}
          <Stack flexDirection={'row'} alignItems={'center'} justifyContent={'center'} columnGap={2}>
            ...
            <Typography variant='h6' fontWeight={300}>{is480?`${userInfo?.name.toString().split(" ")[0]}`:`${userInfo?.name}`}</Typography>
              ...
              !loggedInUser?.isAdmin &&
                  <Stack>
                      <Badge badgeContent={wishlistItems?.length} color='error'>
                          <IconButton component={Link} to={"/wishlist"}><FavoriteBorderIcon sx={{ color: "white" }} /></IconButton>
                      </Badge>
                  </Stack>
            }
            {
              isProductList && <IconButton onClick={handleToggleFilters}><TuneIcon sx={{color:isProductFilterOpen?"black":"white"}}/></IconButton>
            }
